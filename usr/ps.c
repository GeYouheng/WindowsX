...
//       pc_create(2, system_time_proc, (unsigned int)kmalloc(4096), init_gp, "time");
    ...
    else if (kernel_strcmp(ps_buffer, "execk8") == 0) 
    {
        ...
        result = exec_from_kernel(0, (void*)param, 0, 8, 0, 8);
        kernel_printf("execk return with %d\n", result);
        print_all_pcs();
    }
    else if (kernel_strcmp(ps_buffer, "execk7") == 0) 
    {
        kernel_printf("Create a higher priority task, than decrease it, and increase it again.\n");
        result = exec_from_kernel(0, (void*)param, 0, 7, 0, 7);
        kernel_printf("execk return with %d\n", result);
        print_all_pcs();
    } 
